...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Render compact REASSEMBLE pilot audit cards from sampled JSONL and extracted frames.")
...
def side_notes(item: dict[str, Any]) -> list[str]:
    lines: list[str] = []
    high = str(item.get("high_level_text_raw", "")).strip()
    obj = str(item.get("object_raw", "")).strip()
    if high:
        lines.append(f"High-level action: {high}")
    if obj:
...
def write_card(output_dir: Path, item: dict[str, Any], frame_dir: Path, item_index: int) -> Path:
    ...
    title = f"REASSEMBLE Pilot Card {item_index:03d}"
    ...
    draw.text((CARD_MARGIN + 18, y + 14), "Benchmark GT", font=heading_font, fill=CARD_INK)
