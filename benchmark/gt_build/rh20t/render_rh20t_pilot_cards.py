...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Render compact RH20T pilot audit cards from sampled JSONL and extracted frames.")
...
def resolve_frame_paths(item: dict[str, Any], frame_dir: Path) -> list[Path]:
    recording_id = str(item.get("recording_id") or item.get("scene_dir") or item.get("task_id") or "").strip()
...
def frame_labels(item: dict[str, Any], num_frames: int) -> list[str]:
    ...
    if task_type == "T_binary":
        ...
        return labels[:num_frames]
    if isinstance(item.get("frame_indices"), list) and "frame_index" in item:
...
def side_notes(item: dict[str, Any]) -> list[str]:
    ...
    scene_dir = str(item.get("scene_dir", "")).strip()
    if task_desc:
        lines.append(f"Task: {task_desc}")
    if scene_dir:
        lines.append(f"Scene: {scene_dir}")
    if "rating" in item:
        lines.append(f"Rating: {item['rating']}")
    if "calib_quality" in item:
        lines.append(f"Calib quality: {item['calib_quality']}")
    ...
    if "interval_id" in item:
        lines.append(f"Interval: {item['interval_id']}")
    if "scene_success" in item:
        lines.append(f"Scene success: {item['scene_success']}")
    if "auto_tcp_direction_text" in item:
        lines.append(f"Auto tcp direction candidate: {item['auto_tcp_direction_text']}")
    if "delta_x" in item and "delta_y" in item:
        lines.append(f"tcp delta x/y: {item['delta_x']} / {item['delta_y']}")
    if "direction_purity_ratio" in item:
        lines.append(f"Direction purity ratio: {item['direction_purity_ratio']}")
    return lines
...
def build_visual_strip(item: dict[str, Any], frame_paths: list[Path]) -> Image.Image:
    ...
    if num_frames <= 1:
        ...
        panel_w = (available_w - (num_frames - 1) * FRAME_GUTTER) // num_frames
        panel_h = 430 if num_frames == 4 else 390
...
def write_card(output_dir: Path, item: dict[str, Any], frame_dir: Path, item_index: int) -> Path:
    ...
    title = f"RH20T Pilot Card {item_index:03d}"
    scene_dir = str(item.get('scene_dir') or item.get('recording_id') or item.get('task_id') or '')
    meta = f"{item.get('task_type', '')} | scene={scene_dir} | camera={item.get('camera', '')}"
    ...
    stem = f"{item_index:03d}_{item.get('task_type', 'UNK')}_{scene_dir}".replace("/", "_")
        ...
        manifest_rows.append(
            {
                ...
                "scene_dir": item.get("scene_dir") or item.get("recording_id") or item.get("task_id") or "",
